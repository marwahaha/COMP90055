...
public static void main(String[] args) throws MalformedURLException, TwitterException{
		...
		do {			
			ANZFollowers = twitter.getFollowersIDs("ANZ_AU", cursor, 20);
			long[] ANZIDs = ANZFollowers.getIDs();
			
			//get tweets for each of the 20 followers per loop
			for (int i=0; i<ANZIDs.length; i++) {
				int page = 1;
				int count = 50;
				Date collectTime = Calendar.getInstance().getTime();
				long userID = ANZIDs[i];
		...
		} while ((cursor = ANZFollowers.getNextCursor()) != 0);
