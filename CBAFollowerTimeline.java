...
public class CBAFollowerTimeline {
public static void main(String[] args) throws MalformedURLException, TwitterException{
    	...
    	cb.setDebugEnabled(true)
    	  .setOAuthConsumerKey("64eehguktdcibg0V6B63*****")
    	  .setOAuthConsumerSecret("mDEVNM5yOI35MP8Twg9jNDwf3t2pdyzslzUpY6mJDnLd6*****")
    	  .setOAuthAccessToken("1089140644038160385-dQ3wC7yQmuyW2T9Zxd10C8aEu*****")
    	  .setOAuthAccessTokenSecret("8FHvLQWrQCofBCtOlK1F8K8qUAVByWg2URTlOGqC*****");
		...
		do {
			CBAFollowers = twitter.getFollowersIDs("CommBank", cursor, 20);
			long[] CBAIDs = CBAFollowers.getIDs();
			
			//get tweets for each of the 20 followers per loop
			for (int i=0; i<CBAIDs.length; i++) {
				int page = 1;
				int count = 50;
				Date collectTime = Calendar.getInstance().getTime();
				long userID = CBAIDs[i];
		...
		} while ((cursor = CBAFollowers.getNextCursor()) != 0);
